...
終了操作は物理削除ではありません。習慣本体と過去の達成記録を保持します。
...
`habits.archived_at`が存在しない旧DBでは、起動時に軽量マイグレーションを実行します。

1. `archived_at DATETIME NULL`を追加
...
終了済み習慣の終了日は`archived_at`から判定します。終了後に名前を編集してもレポートの対象期間は変わりません。

### 再開後の制約

現行モデルは終了・再開の状態変更履歴を複数区間として保存していません。

一度終了した習慣を再開した場合、レポートでは作成日から現在までを連続した有効期間として扱います。終了中の空白期間を厳密に除外する機能は未対応です。
...
旧DBを直接指定した場合、バックアップ前に`archived_at`の互換移行を実行します。

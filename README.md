...
- `/exports/time-entries.csv` は当月の時間記録をCSVで出力します
- `target_month`には `YYYY-MM` 形式で対象月を指定します
...
- 出力列は日付・カテゴリ・時間（分）・メモ・登録日時です
- ファイル名は `home-panel-time-entries-YYYY-MM.csv` です
- Excelで文字化けしにくいUTF-8 BOM付きで出力します
- カンマ・改行・ダブルクォートは標準CSVとしてエスケープします
- `=`, `+`, `-`, `@`などから始まるメモはCSV数式注入を避けるため先頭にアポストロフィを付与します
- 現在月は当日までを出力し、未来日データを含めません
- 記録がない月はヘッダー行だけを出力します
- 不正な月形式・未来月は400エラーにします
- 応答には `Cache-Control: no-store` と `X-Content-Type-Options: nosniff` を付与します
...
- 月次集計・CSVエクスポート
  ...
  - 未来月は400
  - 月次画面はエラー理由を画面内表示し、CSVエクスポートはテキスト応答を返す
- カード並び替え
...
- CSVエクスポート: BOM、列順、対象月絞り込み、空データ、特殊文字、数式注入対策、レスポンスヘッダー

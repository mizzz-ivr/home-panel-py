...
一括達成・一括取り消しは、単件更新処理をループせず専用サービスで処理します。
...
- 達成一括操作: `complete_expected` / `clear_all`だけ許可
...
- 習慣追加・達成・取り消し・終了
- 過去日の達成追加・取り消し・冪等性
- 対象習慣の一括達成・全達成記録の一括取り消し
- 一括操作の対象曜日・停止期間・終了済み期間判定
- 一括操作による不整合記録の修復
- 一括操作の再送・コミット回数・表示条件
...
- 習慣を選択する部分一括編集・複数日一括編集・月次カレンダーからの直接編集
- 有効期間・曜日設定期間の手動編集
- 日別履歴CSV・年次集計
- キーボード操作によるカード並び替え
- ダッシュボードの表示密度・テーマ設定

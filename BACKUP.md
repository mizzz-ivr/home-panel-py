...
`home_panel.db`に保存されている以下の業務データを、可読性と将来の移行性を考慮したJSON形式で一括保存します。
...
ダッシュボードの並び順・表示設定を保存する`app_settings`は、現時点ではバックアップ対象外です。
...
新しく作成するバックアップはv2です。v1ファイルをv2へ書き換える処理は行いません。
...
v2の検証に成功すると、次のように件数とSHA-256が表示されます。

```text
バックアップは有効です。
レコード件数: ToDo=1、メモ=1、時間記録=1、習慣=1、習慣達成=1
...
- `schema_version`がv1またはv2であること
- アプリケーション識別子が一致すること
- スキーマ版ごとの必須項目が揃い、未知の項目がないこと
- `record_counts`と実データ件数が一致すること
- 各テーブル内でIDが重複していないこと
- 日付・UTC日時が実在すること
- タイトル、メモ、時間、カテゴリが現行の入力制約を満たすこと
- ToDo・習慣の更新日時が作成日時より前になっていないこと
- 習慣達成記録が存在する習慣IDを参照していること
- 同じ習慣・同じ日付の達成記録が重複していないこと
...
この方法では、件数不一致・重複ID・参照不整合・入力制約違反・未対応スキーマなどは検出できません。

Windows PowerShellでは、実際に作成されたファイルパスを指定してください。

## 対象範囲

このバックアップは、対象テーブルに存在する全レコードを保存します。

- アーカイブ済み習慣も保存します。
- 過去の習慣達成記録も保存します。
- 集計画面やCSV出力とは異なり、未来日として保存された時間記録も欠落防止のため対象です。
- `app_settings`は操作環境設定のため対象外です。
...
復元機能を追加する際は、専用CLIによる検証、スキーマ版確認、入力バリデーション、外部キーの復元順序、重複時の方針、トランザクション、復元前バックアップを必須要件として設計してください。
